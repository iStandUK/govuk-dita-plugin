...
| OQ-13 | **Structured data and social metadata** ([12](12-structured-data.md)). Should `govuk.metadata=basic` (canonical, Open Graph, social card, prev/next, Dublin Core) be on by default? May personal authors ever appear (proposal: only under `govuk.metadata.persons=yes`)? Emit `HowTo` and `DefinedTermSet` although no search feature rewards them (proposal: yes; skip `FAQPage`)? How does a publication declare itself a `Dataset`? Does `llms.txt` join this work or follow separately? Is the DCAT catalogue record (§5a) its own typed switch, and which UK profile's mandatory fields does CI check? For data dictionaries, code lists and registers (§5b): DITA conventions first (`data`, `category`, `glossentry`, `subjectScheme`, `resourceid`) or a specialisation now? | `basic` default / off until asked; persons on request; opt-in mechanism for `Dataset`; `govuk.dcat` typed by the publisher | Scope and defaults of R9 |
| OQ-14 | **Page numbers in print — product shape** ([13](13-print-page-numbers.md)). Amend D-20 so the CSS engine, fonts and PDF/UA validation ship as a companion plugin `org.istanduk.gov-uk.pdf` on the unchanged core (recommended), or keep them inside the core? Ship the external-formatter hook `govuk.pdf.command` in a 1.0.x release first? One repository with two plugin directories, or two repositories? Versions in lockstep or independent with a `deps` range? Is reader-side pagination ever offered? | companion product / bundled in core; `govuk.pdf.command` now / with 1.1; monorepo / two repos | FR-P3 scope, 1.1 sizing, release process |
