...
**Consequences:** decision 4 is expected to take time, so the work that waits on it was separated from the work that does not (2026-09-08): it blocks the DCAT record ([#102](https://github.com/iStandUK/govuk-dita-plugin/issues/102)), schema.org `Dataset` ([#122](https://github.com/iStandUK/govuk-dita-plugin/issues/122)) and ADMS typing of publication parts ([#123](https://github.com/iStandUK/govuk-dita-plugin/issues/123)) — the last two split out of [#101](https://github.com/iStandUK/govuk-dita-plugin/issues/101) and [#103](https://github.com/iStandUK/govuk-dita-plugin/issues/103), which no longer depend on it. Everything else proceeds: page metadata ([#99](https://github.com/iStandUK/govuk-dita-plugin/issues/99)), the JSON-LD core ([#100](https://github.com/iStandUK/govuk-dita-plugin/issues/100)), `HowTo`/`DefinedTermSet` (#101) and SKOS concept schemes (#103). Requirements **FR-D1–FR-D4** are added to [02](02-requirements.md); `govuk.dcat` is no longer a type list but an on/off switch over declarations the DITA carries, with its final shape settled by decision 4.
