...
| Criterion | A external | B reader-side | C bundled in core | D companion product |
|---|---|---|---|---|
| Page numbers in contents, index, cross-references | yes (tool) | yes, at print time | yes | yes |
| Footnotes at the page foot, running heads | yes (tool) | partly | yes | yes |
| PDF/UA, validated | tool-dependent | no (browser's PDF) | yes, veraPDF in CI | yes, veraPDF in the companion's CI |
| Publisher dependency | a formatter | none | none | one more `dita install` |
| Reader cost | none | JavaScript on every print | none | none |
| Core plugin size / licences | unchanged | +1 MB JS in output | ×20–40, LGPL + fonts inside | unchanged |
| Classpath risk with the toolkit's PDFBox 3.0.5 | none | none | real unless forked | isolated by design (forked JVM, own classpath) |
| Determinism (FR-B6) | tool-dependent | n/a | engine gives fixed dates; byte-identical achievable | same |
| Release cadence | core | core | engine churn hits the core | companion tracks the engine; core untouched |
| Effort | **S** (3–4 days with CI proof) | S–M | **L** (3–5 weeks after a spike) | **L** + ~1 week of product plumbing |
...
1. Amend D-20 to the two-product shape (recommended), or keep the engine inside the core.
